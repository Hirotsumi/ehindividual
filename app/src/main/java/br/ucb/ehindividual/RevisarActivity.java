...
public class RevisarActivity extends Activity implements View.OnClickListener, DialogInterface.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (questoes != null) {
            for (int j = 0; j < questoes.length; j++)
                questoesErradas[j] = pontosJogador1[j] == 0 || pontosJogador2[j] == 0;
        }
        carregarQuestao();
    }
    ...
    private void inicializarVariaveis() {
        ...
        questoesErradas = new boolean[questoes.length];
    ...
    private void carregarQuestao() {
        if (i < questoes.length && questoesErradas[i]) {
            ...
            if (pontosJogador1[i] == 10) {
                rgJogador1.setVisibility(View.INVISIBLE);
                tvNomeJogador1.setVisibility(View.INVISIBLE);
            }else {
                rgJogador1.setVisibility(View.VISIBLE);
                tvNomeJogador1.setVisibility(View.VISIBLE);
            }
            if (pontosJogador2[i] == 10){
                rgJogador2.setVisibility(View.INVISIBLE);
                tvNomeJogador2.setVisibility(View.INVISIBLE);
            }else{
                rgJogador2.setVisibility(View.VISIBLE);
                tvNomeJogador2.setVisibility(View.VISIBLE);
            }
        }else{
    ...
    private void gravarRespostas() {
        ...
        boolean jaTinhaAcertado1 = respostaJogador1 == -1;
        boolean jaTinhaAcertado2 = respostaJogador2 == -1;
        ...
        if (jogador1Acertou && jogador2Acertou) {
            ...
            alert.setMessage(nomeJogador2 + " acertou a questão!");
        }else if (jogador1Acertou && !jogador2Acertou) {
    ...
    private boolean validarRespostas() {
        if (rgJogador1.getVisibility() == View.VISIBLE && rgJogador1.getCheckedRadioButtonId() == -1) {
            mostrarMensagem(nomeJogador1 + " deve selecionar uma resposta");
            return false;
        }
        if (rgJogador2.getVisibility() == View.VISIBLE && rgJogador2.getCheckedRadioButtonId() == -1) {
